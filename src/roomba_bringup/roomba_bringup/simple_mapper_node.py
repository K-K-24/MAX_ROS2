...
class SimpleMapperNode(Node):
	...
	def __init__(self):
		...
		self.get_logger().info(f'📐 Map setup: {self.height_cells}x{self.width_cells} cells, cell_size={self.cell_side}m')
	...
	def mapper_callback(self):
		...
		new_grid_x = round(self.grid_x + diff_y)        # Adding the difference smartly to get the new coordinates of the grid_cell
		new_grid_y = round(self.grid_y + diff_x)
		...
		if new_grid_x != self.grid_x or new_grid_y != self.grid_y:
			self.get_logger().info(f'⚠️  Clamped position: ({new_grid_x},{new_grid_y}) → ({self.grid_x},{self.grid_y})')
		...
		if not not_moving:
			next_x_raw = self.grid_x + round(math.sin(self.theta))   # Always finding the coord of the next cell with bounds checking
			next_y_raw = self.grid_y + round(math.cos(self.theta))		# dsin and dcos for the distance - d( Cell size)

			self.get_logger().info(f'👀 Next cell calculation: ({self.grid_x}, {self.grid_y}) + ({round(math.sin(self.theta))}, {round(math.cos(self.theta))}) = ({next_x_raw}, {next_y_raw})')
			...
			if self.is_valid_cell(next_x_raw, next_y_raw):
				...
				if self.obstacle_dist > self.cell_side:
					...
					self.get_logger().info(f'✅ Updated Current map cell ({self.next_cell_x}, {self.next_cell_y}) as Free space')
				else:
					self.map_array[self.next_cell_x, self.next_cell_y] = 100
					self.get_logger().info(f'🚧 Updated Current map cell ({self.next_cell_x}, {self.next_cell_y}) as Obstacle')
